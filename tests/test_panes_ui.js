...
assert.equal(run('paneGeometry(1440,defaultPanes()).widths.workspace'),44);
assert.equal(run('paneGeometry(1440,defaultPanes()).widths.assistant'),1160);
...
assert.equal(run('cleanPanePreferences({navigation:-100,assistant:"bad"}).assistant'),460);
assert.equal(run('paneGeometry(760,defaultPanes()).closed.assistant'),false);
...
assert.equal(run('defaultPanes().collapsed.assistant'),false);
