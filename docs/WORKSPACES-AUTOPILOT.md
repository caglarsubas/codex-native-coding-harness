...
- [x] WSP-03G: [owner rereview controls](REREVIEW-CONTROLS.md), selected-workspace/task inspection and immutable history, signed authorize/revoke previews and exact transaction-bound confirmation in Workers & evidence. No result acceptance, native resume, notification, accounting reset or Play; assistant receives cached closed metadata only.
...
**WSP-05F** adds [owner retention controls](RETENTION-CONTROLS.md) to the selected
workspace. Explicit inspection shows saved policy versions, per-run attempts and
review blockers. Same-session signed five-minute previews require owner confirmation
and a separate cleanup acknowledgment; transaction-bound retries never restore a
revoked policy. Revocation cannot undo an already-consumed native send boundary.
The assistant receives cached historical metadata and links only. This supersedes
the internal-owner-only UI limitation above, not preservation, host, Pause or Play
gates. No live installation, policy review, archival, scheduler or billing change.
...
- [x] WSP-05F: selected-workspace owner inspection/history and signed review/revoke confirmations, explicit cleanup consent, bounded cached assistant facts/navigation and two-workspace UI isolation. Owner controls are source-qualified; complete output/descendant preservation, host evidence and live acceptance remain gated.
...
- [x] WSP-04G: authenticated owner model-policy controls in Mission & authority with explicit recorded-catalog choices, quality/token floors, escalation bounds, signed previews and atomic review/revoke. Source/fixtures only; closes the WSP-04E dashboard gap, not live model qualification, speed/Ultra, run activation or Play. See [owner workflow](MODEL-POLICY-CONTROLS.md).
- [x] WSP-04H: authenticated observation-endpoint owner controls and saved report metadata in Run readiness, exact existing allocation/file bindings and original-time history. Source/fixtures only; closes the WSP-04F owner UI gap without native connection, collection, startup, accounting mutation or Play. Complete task/counter/cleanup/telemetry evidence remains unqualified. See [owner workflow](NATIVE-OBSERVER-CONTROLS.md).
