...
- [x] WSP-04G source after verified PR #57: owner model/effort policy controls in
  ...
  Closes WSP-04E's owner dashboard gap and advances milestone 6 without switching
  native settings, granting run authority or starting Play. Conditional **8–12 /
  10–16** ranges remain unchanged for this partial milestone; complete native
  lifetime counters and process-tree cleanup remain an external capability gate.
  See [saved plan and owner workflow](MODEL-POLICY-CONTROLS.md). No live rollout,
  provider call, notification, schedule change or Actions execution.
  Local verification: 1,513 Python tests, all fourteen JavaScript UI suites,
  JavaScript syntax, Python compilation and diff whitespace checks passed;
  disposable browser review/revoke/history and two-workspace isolation verified.
