...
- [x] WSP-03E: selected-workspace owner checkpoint review/withdrawal with signed expiring previews, exact next-mission/settings binding, durable withdrawal and grant-race checks; historical retries never reapply permission (source/fixtures, not phase acceptance, run authorization or Play; [plan](CHECKPOINT-CONTROLS.md))
- [x] WSP-03F: internal exact-owner later-generation review-only permission for settled standard results, same-commit recollection/rereview and immutable prior outcome verification (source/fixtures; no accepted-result reopening, native correction, accounting reset, public owner adapter or Play; [plan](GENERATION-RESULT-REVIEW.md))
- [x] WSP-03G: selected-workspace/task owner rereview controls in Workers & evidence, explicit retained-version inspection, signed authorize/revoke previews and atomic confirmation; cached assistant metadata only (local/disposable browser fixtures, not acceptance, native resume, notification, accounting reset or Play; [owner workflow](REREVIEW-CONTROLS.md))
...
- [x] WSP-04D7: explicit selected-workspace budget inspection, shared arithmetic, phase charges/reservations/checkpoint reserve, separate account windows/global recorded counts, unknown/stale balance suppression and cached overview/assistant context (read-only saved evidence; not native telemetry, billing, free capacity or live activation; [plan](BUDGET-VISIBILITY.md))
...
- [x] WSP-05E: opt-in exact-run owner retention policy, capped designated-brain archive requests and existing one-shot accepted/preserved root-task lifecycle; revocation/supersession, immutable attempts and historical late-outcome guards (fixtures only; no public policy control, native archive, complete descendant/output retention or Play; [plan](DELEGATED-RETENTION.md))
- [x] WSP-05F: selected-workspace retention inspection/history, signed same-session review/revoke previews and explicit owner/cleanup confirmation; unchanged one-shot archive gates, workspace-isolated drafts and cached assistant facts/navigation only (local and disposable-browser fixtures, not live policy, native archival or Play; [owner workflow](RETENTION-CONTROLS.md))
...
- [x] WSP-04G: selected-workspace owner model-policy inspection, explicit model/effort and quality/token choices, signed review/revoke confirmation and original-time history in Mission & authority; stale capabilities block review and identical retries never restore revoked policy (local/disposable browser fixtures; no provider/native calls, settings change, run activation or Play; [owner workflow](MODEL-POLICY-CONTROLS.md))
- [x] WSP-04H: owner observation-endpoint inspection and signed review/revoke in Run readiness, existing allocation binding, lossless file fingerprints and saved historical collection metadata (local/disposable fixtures; no connection, collection, startup, notification, usage change or Play; [owner workflow](NATIVE-OBSERVER-CONTROLS.md))
