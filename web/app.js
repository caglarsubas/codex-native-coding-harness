...
function operations(root) {
 ...
 const split=el("div",null,"split"),left=el("section"),right=el("section");left.append(section("Brain checkpoint",age(m.lastReconciled)),narrative(m.checkpoint,'Brain checkpoint'));right.append(section("Control state"));right.append(table(["Setting","Value"],[["Heartbeat",m.heartbeat.status],["Pilot",m.pilotPassed?"Verified · two slots enabled":"Pending · one slot limit"],["Controller",m.controller?m.controller.owner:"No active cycle"],["Revision",m.revision]]));split.append(left,right);root.append(split);
}
function queue(root) {
