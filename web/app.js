...
function metrics(root) {
 const summary=state.summary.aggregate;root.append(callout("Measured text, not a productivity score", "Counts include blank and comment lines in tracked UTF-8 files. Vendor/build/generated folders, lockfiles, binaries, symlinks and files over 2 MiB are excluded. Each repository is counted once, at its recorded commit."));
 const strip=el("div",null,"summary-strip");[[num(summary.lines),"physical lines"],[num(summary.characters),"Unicode characters"],[num(summary.files),"text files"],[summary.measuredRepositories+" / "+summary.repositoryCount,"repos measured"]].forEach(([v,l])=>{const s=el("div");s.append(el("strong",v),el("span",l));strip.append(s);});root.append(strip);
 root.append(section("Repository distribution","Snapshots are refreshed by the brain on request, not on every UI poll."));root.append(table(["Repository / commit","Source lines","Test lines","Docs lines","All text lines","Characters","Status"],state.summary.repositories.map(m=>[textCell(m.repository,m.commit?.slice(0,12)||m.reason),num(m.groups.source?.lines),num(m.groups.tests?.lines),num(m.groups.docs?.lines),m.status==='measured'?num(m.lines):"—",m.status==='measured'?num(m.characters):"—",badge(m.status)])));
...
function render() {if(!state)return;document.querySelector(".page-actions").hidden=view==="workspaces";const m=state.meta,dispatch=dispatchPresentation(m,state.commands);$('mode').textContent=dispatch.label+" · Brain: "+activityLabel(state.brainActivity)+" · Checkpoint: "+age(m.lastReconciled)+" · Heartbeat (recorded): "+m.heartbeat.status;$('pause').textContent=dispatch.button;$('pause').disabled=!connected||busy||dispatch.disabled;$('reconcile').disabled=!connected||busy;$('title').textContent=titles[view][0];$('subtitle').textContent=titles[view][1];const root=$('content');root.replaceChildren();({overview,decisions,queue,workers,knowledge,metrics,usage,gitStatus,artifacts,roadmap,readiness,workspaces:allWorkspaces})[view](root);if(view!=="workspaces"&&state.commands.length){root.append(section("Control requests","Delivery, brain receipt and completion are separate."));root.append(table(["Request","Status","Result"],[...state.commands].reverse().slice(0,8).map(c=>{const delivery=commandPresentation(c);return [textCell(c.kind,when(c.createdAt)),badge(delivery.label),delivery.detail];})));}}
