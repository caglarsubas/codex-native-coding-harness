...
function render() {
 if(!state)return;
 document.querySelector(".page-actions").hidden=['workspaces','mission','runReadiness','phaseCheckpoints'].includes(view);
 ...
 ({overview,decisions,queue,workers,knowledge,metrics,usage,gitStatus,artifacts,roadmap,readiness,mission:missionView,runReadiness:runReadinessView,phaseCheckpoints:phaseCheckpointsView,workspaces:allWorkspaces})[view](root);
 if(!['workspaces','mission','runReadiness','phaseCheckpoints'].includes(view)&&state.commands.length){root.append(section("Control requests","Delivery, brain receipt and completion are separate."));root.append(table(["Request","Status","Result"],[...state.commands].reverse().slice(0,8).map(c=>{const delivery=commandPresentation(c);return [textCell(c.kind,when(c.createdAt)),badge(delivery.label),delivery.detail];})));}
}
