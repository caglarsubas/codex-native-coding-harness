...
function observationFilters(root) {
  ...
  [["all", "All repositories"], ...(view==='artifacts'?[["@portfolio","Portfolio-wide briefs"]]:[]), ...state.repositories.map(r => [r.id, r.id])].forEach(([id, name]) => {
...
function artifacts(root) {
  ...
  paginated(root, rows, ["Artifact / repository", "Version", "Order date / basis", "Captured", "Task references"], a => {
    const generated=a.provenance==='local_inference_advisory';
    ...
    return [name, "v" + a.version, textCell(when(a.orderAt), generated?"Generated by inference":a.createdAt ? "Supplied creation time" : a.references.length ? "First retained reference" : "First observed"), when(a.observedAt), [...new Set(a.references.map(r => r.session).filter(Boolean))].join(", ") || (generated?'Advisory · not a native task':"Not linked")];
  });
