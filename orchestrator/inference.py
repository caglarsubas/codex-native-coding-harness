...
def projection(state):
    ...
    facts = [
        ...
        {"id": "F2", "label": "Repository readiness", "data": {"configured": len(repos), "nativeProjectMapped": sum(bool(r.get("projectId")) for r in repos),
            ...
            "codeMeasuredRepositories": summary["aggregate"]["measuredRepositories"],
            "codeNotMeasuredRepositories": len(repos) - summary["aggregate"]["measuredRepositories"]}},
        {"id": "F3", "label": "Development size at recorded commits", "data": {k: summary["aggregate"][k] for k in ("files", "lines", "characters")}},
